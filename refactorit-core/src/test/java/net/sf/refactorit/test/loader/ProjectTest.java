...
public class ProjectTest extends RitTestCase {
  ...
  private static final Category cat =
      Category.getInstance(ProjectTest.class.getName());
...
//    }
  ...
  public void testLastRebuildTime() throws Exception {
    ...
    Thread.sleep(50); // sometimes rebuild takes ~0 time
  ...
  public void testRebuildBinaryClasses() throws Exception {
    ...
    final Project project = Utils.createTestRbProject(
        Utils.getTestProjects().getProject("RebuildBinaryClasses"));
    project.getProjectLoader().build();
    ...
    assertTrue("class E is Ok before rebuild",
        beforeRebuild.getBinCIType() != null
        && !(beforeRebuild.getBinCIType() instanceof MissingBinClass)
        && !(beforeRebuild.getBinCIType() instanceof MissingBinInterface));
    ...
    assertTrue("class E is Ok after rebuild",
        afterRebuild.getBinCIType() != null
        && !(afterRebuild.getBinCIType() instanceof MissingBinClass)
        && !(afterRebuild.getBinCIType() instanceof MissingBinInterface));

    // switched off that deep discovery of binary types
  ...
  public void testRebuildLocalsOfLocals() throws Exception {
    ...
    assertTrue("class B is Ok after rebuild",
        afterRebuild.getBinCIType() != null
        && !(afterRebuild.getBinCIType() instanceof MissingBinClass)
        && !(afterRebuild.getBinCIType() instanceof MissingBinInterface));

    assertTrue("project has errors after rebuild",
  ...
  public void testClearingOfLocalOfLocalOnIncrementalRebuild() throws Exception {
    ...
    Project project = Utils.createTestRbProject(
        Utils.getTestProjects().getProject("IncrementalRebuildLocalsOfLocals"));
    ...
    assertTrue("class YYY is Ok before rebuild",
        beforeRebuild.getBinCIType() != null
        && !(beforeRebuild.getBinCIType() instanceof MissingBinClass)
        && !(beforeRebuild.getBinCIType() instanceof MissingBinInterface));

    assertTrue("class YYY has subclasses before rebuild",
        beforeRebuild.getDirectSubclasses().size() > 0);
    ...
    Writer writer = new BufferedWriter(new OutputStreamWriter(
        ...
        GlobalOptions.getEncoding()), content.length() + 8);
    ...
    assertTrue("project must have no errors after rebuild",
        !(project.getProjectLoader().getErrorCollector()).hasUserFriendlyErrors()
        && !(project.getProjectLoader().getErrorCollector()).hasUserFriendlyInfos());

    BinTypeRef afterRebuild = project.getTypeRefForName("YYY");
    assertNotNull("must find class YYY after rebuild", afterRebuild);
    assertTrue("class YYY is Ok after rebuild",
        afterRebuild.getBinCIType() != null
        && !(afterRebuild.getBinCIType() instanceof MissingBinClass)
        && !(afterRebuild.getBinCIType() instanceof MissingBinInterface));

    assertTrue("class YYY has no subclasses after rebuild",
        afterRebuild.getDirectSubclasses().size() == 0);

    cat.info("SUCCESS");
  }

  public void testBug2021() throws Exception {
    // TODO JSpinner was introduced in jdk-1.4 and won't run on karkass
    // make similar class, but put not sources but binary into CVS!
    if (!RefactorItConstants.runNotImplementedTests) {
      return;
    }
    ...
    // Builds method bodies
    {
      CompilationUnit f = p.getCompilationUnitForName("X.java");
      ((BinTypeRef) f.getDefinedTypes().get(0)).getBinCIType().accept(
          new BinItemVisitor());
    }

    assertFalse(
        (p.getProjectLoader().getErrorCollector()).hasUserFriendlyErrors() ? (p.getProjectLoader().getErrorCollector()).getUserFriendlyErrors().next().toString() : null,
        (p.getProjectLoader().getErrorCollector()).hasUserFriendlyErrors());
  }
  ...
  public void testBug2057() throws Exception {
    ...
    final Project project = Utils.createTestRbProject(
        Utils.getTestProjects().getProject("bug #2057"));
    project.getProjectLoader().build();
    project.discoverAllUsedTypes();
    ...
    project.getProjectLoader().forceSourceModified(
        rebuildableBefore.getCompilationUnit().getSource());
    project.getProjectLoader().build(null, false);
    ...
    assertNotSame("Rebuildable type must have been rebuilded!",
        rebuildableBefore, rebuildableAfter);

    assertNotNull("Super constructor has an owner after rebuild",
        ((BinConstructorInvocationExpression) ((BinExpressionStatement) ((
        BinClass) testAfter).getDeclaredConstructors()[0]
        .getBody().getStatements()[0])
        .getExpression()).getConstructor().getOwner());
  ...
  public void testBugRIM708() throws Exception {
    ...
    try {
      ...
      CompilationUnit compilationUnit
          = (CompilationUnit) p.getCompilationUnits().get(0);
      ASTTree tree = compilationUnit.getSource().getASTTree();

      assertEquals("Types ok: " + tree.getTypeNames(),
          new HashSet(Arrays.asList(new String[] {"A", "AAA", "Wuuu"})),
          tree.getTypeNames());

      assertEquals("Type full names ok: " + tree.getTypeFullNames(),
          new HashSet(Arrays.asList(new String[] {"Wuuu", "Wuuu$A", "Wuuu$A$AAA"})),
          tree.getTypeFullNames());

      assertEquals("Super types ok: " + tree.getSuperTypeNames(),
          new HashSet(Arrays.asList(new String[] {"B", "C", "D", "E", "K", "L", "M", "N", "O"})),
          tree.getSuperTypeNames());
    } finally {
