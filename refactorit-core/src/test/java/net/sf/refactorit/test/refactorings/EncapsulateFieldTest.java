...
public class EncapsulateFieldTest extends RefactoringTestCase {
  ...
  private static final Category cat =
      Category.getInstance(EncapsulateFieldTest.class.getName());
  ...
  private void performEncapsulate(String typeName,
      String fieldName,
      String getterName,
      String setterName,
      boolean preConditionsWarningExpected,
      boolean userInputWarningsExpected) throws Exception {
    performEncapsulate(typeName, fieldName, getterName, setterName,
        preConditionsWarningExpected, userInputWarningsExpected, -1, true, true);
  }
  ...
  private void performEncapsulate(String typeName,
      ...
      String setterName,
      boolean preConditionsWarningExpected,
      boolean userInputWarningsExpected,
      int fieldVisibility) throws Exception {
    performEncapsulate(typeName, fieldName, getterName, setterName,
        preConditionsWarningExpected, userInputWarningsExpected,
        fieldVisibility, true, true);
  }
  ...
  private void performEncapsulate(String typeName,
    ...
    final EncapsulateField encapsulator = new EncapsulateField(
        new NullContext(project), field);

    RefactoringStatus status = encapsulator.checkPreconditions();
    assertTrue("Preconditions check status is ok: "
        + status.getAllMessages(),
        preConditionsWarningExpected ? !status.isErrorOrFatal() : status.isOk());
    ...
    assertTrue("User input check status is ok: "
        + status.getAllMessages(),
        userInputWarningsExpected ? !status.isErrorOrFatal() : status.isOk());

    status = encapsulator.apply();

    assertTrue("Encapsulated successfully: "
        + status == null ? "" : status.getAllMessages(),
        status == null || status.isOk());

    RwRefactoringTestUtils.assertSameSources(
        "Encapsulated field", getExpectedProject(), project);
  ...
  public void testCustomAccessorName() throws Exception {
    performEncapsulate("A", "test", "get_test", "set_test", false, false,
  ...
  public void testbug1527() throws Exception {
    performEncapsulate("A", "test", "getTest", "setTest", false, true,
        BinModifier.PRIVATE);
  }
