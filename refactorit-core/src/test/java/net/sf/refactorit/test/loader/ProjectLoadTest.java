...
public class ProjectLoadTest extends TestCase {
  ...
  private static final Category cat =
      Category.getInstance(ProjectLoadTest.class.getName());
  ...
  public static Test suite() throws Exception {
    ...
    for (final Iterator i = Utils.getTestProjects().getProjects().iterator();
        i.hasNext(); ) {
      final ProjectMetadata data = (ProjectMetadata) i.next();
      
      if (data.isTestForLoad() 
          // RefactorIT load is slow, temporarly disabled 
          && !"RefactorIT".equals(data.getId())
          ) {
  ...
  public static void processProject(Project project) throws Exception{
    ...
    cat.debug("Project " + project.getName() + " loaded in "
    ...
    cat.debug("Project " + project.getName() + " indexed in "
        + (timeAfterIndexing - timeBeforeIndexing) + " ms");
  ...
  /***** END Test interface implementation *****/
}
