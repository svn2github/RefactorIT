...
public class WhereCaughtTest extends TestCase {
  ...
  private static final Category cat =
      Category.getInstance(WhereCaughtTest.class.getName());
  ...
  public void testBug2213() throws Exception {
    ...
    BinStatement[] catchStatement = declaredMethods[0].
        getBody().getStatements();
    BinTryStatement tryStatement = (BinTryStatement) catchStatement[0];
    CatchClause[] catches = tryStatement.getCatches();

    declaredMethods = binCIType.getAccessibleMethods("exceptionalMethod",
        binCIType);
    BinStatement[] throwStatement = declaredMethods[0].
        getBody().getStatements();
    ...
    child = (BinTreeTableNode) child.getChildAt(0);
    assertTrue("Comparing get model node with the real node",
        child.getBin() == catches[0]);
