 ...
 * licenses.
 */
package net.sf.refactorit.netbeans.v5.vfs;
...
public class ClassPathElementTest extends TestCase {
  ...
  public void test_getFileSystem() {
    FileObjectVisitor visitor = new FileObjectVisitor() {
      public void accept(Entry entry) throws Exception {
        ...
        assertEquals("FileSystem of the NBClassPathElement shall be the same,"
            + " what was used during its creation.", fs, element
            .getFileSystem());
      }
    };
  ...
  public void test_getAbsolutePath() {
    FileObjectVisitor visitor = new FileObjectVisitor() {
      public void accept(Entry entry) throws Exception {
        ...
        assertEquals("Absolute path displayed is correct.", path, element
            .getAbsolutePath());
      }
    };
    visitProjectsClassPathes(visitor);
  ...
  public void test_ClassPathEntry() {
    // This test failed on a RefactorIT project's lib. But since the code
    // that it tests (NBClassPathElement) is not used under NB 4.0 anyway,
    // we do not have to fix this right now.
    ...
    FileObjectVisitor visitor = new FileObjectVisitor() {
      public void accept(Entry entry) throws Exception {
        FileSystem fs = entry.getRoot().getFileSystem();
        NBClassPathElement element = new NBClassPathElement(fs);
        net.sf.refactorit.vfs.ClassPath.Entry classpathEntry = element
            .getEntry(getName(entry));
        
        assertTrue("Entry must be located on the classPath: " +
             getName(entry), classpathEntry != null);
        assertEquals("Length must be correct: ", fs.findResource(getName(entry))
            .getSize(), classpathEntry.length());
      }
    };
    visitProjectsClassPathes(visitor);
  }
