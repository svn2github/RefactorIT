...
public class PathItemReferenceTest extends NbTestCase {
  ...
  public void testLocalFile() {
    ...
    assertEquals(localFile, reference.getFile());
    assertEquals(localFile.getAbsolutePath(), reference.getAbsolutePath());
    assertEquals(localFile.getAbsolutePath() + PathItemReference.INVALID_ITEM_SUFFIX, 
        reference.getDisplayNameLong());
    assertEquals(localFile.getName() + PathItemReference.INVALID_ITEM_SUFFIX, 
        reference.getDisplayNameWithoutFolders());
  }
  ...
  public void testFileObject() throws IOException {
    ...
    assertEquals(fileObject, ((NBFileObjectClassPathElement)reference.getClassPathElement()).
        getFileObject());
    ...
    assertEquals(absolutePath + " [GOT: " + reference.getAbsolutePath() + "]",
        absolutePath, reference.getAbsolutePath());
    assertEquals(fileObject.getNameExt() + PathItemReference.INVALID_ITEM_SUFFIX, 
        reference.getDisplayNameWithoutFolders());
    assertEquals(absolutePath + PathItemReference.INVALID_ITEM_SUFFIX, 
        reference.getDisplayNameLong());
  ...
  public void testGetDisplayNameWithoutFolders_Root() throws FileStateInvalidException {
    ...
    final String shortDisplayName = RefactorItActions.isNetBeansFour() ? longDisplayName :
        FileUtil.extractFileNameFromPath(longDisplayName, File.separatorChar);
    
    assertEquals(shortDisplayName, 
        new PathItemReference(root).getDisplayNameWithoutFolders());
    assertEquals(longDisplayName, 
        new PathItemReference(root).getDisplayNameLong());
    assertEquals(longDisplayName, 
        new PathItemReference(root).getAbsolutePath());
  }
  ...
  static FileObject createFileObject() {
    FileObject fileObject = ((NBSource)IDEController.getInstance().getActiveProject().getPaths().getSourcePath().getRootSources()[0]).
        getFileObject();
    try {
